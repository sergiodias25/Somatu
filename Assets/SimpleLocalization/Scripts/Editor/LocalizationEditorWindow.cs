...
namespace Assets.SimpleLocalization.Scripts.Editor
{
    public class LocalizationEditorWindow : EditorWindow
    {
        ...
        {
            if (
                Settings.Sheets.Count == 0
                || Editor.SheetNames.Count == 0
                || string.IsNullOrEmpty(Settings.TableId)
            ...
            if (Directory.GetFiles(AssetDatabase.GetAssetPath(Settings.SaveFolder)).Length == 0)
            {
                if ((DateTime.UtcNow - LocalizationSettings.Timestamp).TotalSeconds < 10)
                {
                    ...
                    EditorUtility.DisplayDialog(
                        "Error",
                        "Nothing to edit. You need to download sheets in the Settings window!",
                        "OK"
                    )
                )
                {
        ...
        public void OnDestroy()
        {
            if (Editor.KeysActions.Count > 0)
            {
                if (
                    EditorUtility.DisplayDialog(
                        "Simple Localization",
                        "Do you want to submit changes before exiting?",
                        "Yes",
                        "No"
                    )
                )
                {
                    ...
                    var sheetDictionary = Editor.SheetDictionary.ToDictionary(
                        entry => entry.Key,
                        entry => entry.Value
                    );
                    var keysActions = Editor.KeysActions.ToDictionary(
                        entry => entry.Key,
                        entry => entry.Value
                    );

                    EditorCoroutineUtility.StartCoroutine(
                        SaveSheet(keys, keysActions, sheetDictionary),
                        this
                    );
                }
            }
        ...
        private static void MakeLanguagesRow()
        {
            ...
            GUILayout.Label(
                "Key"
                    + (
                        Editor.CurrentKey == ""
                            ? ""
                            : Editor.Keys[Editor.CurrentKey] != Editor.CurrentKey
                                ? " *"
                                : ""
                    ),
                style,
            ...
            foreach (var language in Editor.SheetDictionary.Keys)
            {
                ...
                GUILayout.Label(
                    language + (language == LocalizedTranslate.PrimaryLanguage ? " [Primary]" : ""),
                    style,
                    GUILayout.MinWidth(_columnWidth - 28),
                    GUILayout.MinHeight(20)
                );
                ...
                if (Event.current.type == EventType.Repaint)
                    LanguageButtons[language] = GUILayoutUtility.GetLastRect();
        ...
        private static void MakeSheetRow(string key)
        {
            ...
            var newValue = EditorGUILayout.TextField(
                ...
                GUILayout.MaxWidth(MinColumnWidth),
                GUILayout.MinWidth(MinColumnWidth + 7)
            );

            if (Editor.Keys[key] != newValue)
            {
                var duplicateKey = Editor.Keys.ContainsKey(newValue)
                    ? Editor.Keys[newValue]
                    : Editor.Keys.Values.Contains(newValue)
                        ? Editor.Keys.First(i => i.Value == newValue).Key
                        : "";

                if (
                    duplicateKey != "" && Editor.KeysActions[duplicateKey] != ActionType.Delete
                    || Editor.Keys.Count(i => i.Value == newValue) > 1
                )
                {
                    EditorUtility.DisplayDialog(
                        "Error",
                        "A key with the same name already exists in the table!",
                        "OK"
                    );
                }
                else
            ...
            foreach (var language in Editor.SheetDictionary.Keys)
            {
                var value = EditorGUILayout.TextArea(
                    Editor.SheetDictionary[language][key],
                    GUILayout.MinHeight(50),
                    GUILayout.MaxWidth(_columnWidth),
                    GUILayout.MinWidth(_columnWidth + 7)
                );

                if (value == Editor.SheetDictionary[language][key])
                    continue;
        ...
        private void MakeKeysDropdown()
        {
            ...
            var keysArray =
                filter.Length > 2
                    ? Editor.Keys.Keys
                        .Where(
                            originalKey =>
                                !IsDeleted(originalKey) && originalKey.ToLower().Contains(filter)
                                || originalKey == Editor.CurrentKey
                        )
                        .ToArray()
                    : Editor.Keys.Keys.Where(originalKey => !IsDeleted(originalKey)).ToArray();
            var valuesArray = Editor.Keys
                .Where(i => keysArray.Contains(i.Key))
                .Select(i => i.Value)
                .ToArray();
            var selectedKeyArrayIndex = string.IsNullOrEmpty(Editor.CurrentKey)
                ? 0
                : Array.IndexOf((Array)keysArray, Editor.CurrentKey);
            ...
            selectedKeyArrayIndex = EditorGUILayout.Popup(
                selectedKeyArrayIndex,
                valuesArray,
                GUILayout.MinWidth(MinColumnWidth + ButtonsColumnSizeFix),
                GUILayout.MaxWidth(MinColumnWidth + ButtonsColumnSizeFix)
            );
            ...
            {
                EditorCoroutineUtility.StartCoroutine(
                    new LocalizedTranslate().TranslateAuto(),
                    this
                );
            }
            ...
            {
                return Editor.KeysActions.ContainsKey(key)
                    && Editor.KeysActions[key] == ActionType.Delete;
            }
        }
        ...
        private string MakeSheetsDropdown(List<string> sheetNames)
        {
            ...
            var newSheetIndex = EditorGUILayout.Popup(
                _selectedSheetIndex,
                sheetNamesArray,
                GUILayout.MinWidth(MinColumnWidth + ButtonsColumnSizeFix),
                GUILayout.MaxWidth(MinColumnWidth + ButtonsColumnSizeFix)
            ...
            if (_selectedSheetIndex == newSheetIndex)
            ...
            if (Editor.KeysActions.Count > 0)
            {
                if (
                    EditorUtility.DisplayDialog(
                        "Simple Localization",
                        "Changes will be lost when switching sheet!\nDo you want to submit changes?",
                        "Yes",
                        "No"
                    )
                )
                {
                    EditorCoroutineUtility.StartCoroutine(
                        SaveSheet(Editor.Keys, Editor.KeysActions, Editor.SheetDictionary),
                        this
                    );
                }
            }
        ...
        public void MakeBottomMenu()
        {
            ...
            if (
                GUILayout.Button("Revert", GUILayout.MaxWidth(MinColumnWidth + 3))
                && EditorUtility.DisplayDialog(
                    "Simple Localization",
            ...
            GUILayout.Label(
                $"Pending submit of {Editor.KeysActions.Count} changes",
        ...
        private IEnumerator SaveSheet(
            Dictionary<string, string> keys,
            Dictionary<string, ActionType> actionTypes,
        ...
        )
        {
            ...
            {
                rows.Add(
                    LocalizationUtils.CreateRow(action.Key, action.Value, keys, sheetDictionary)
                );
            }
            ...
            yield return LocalizationUtils.SubmitChanges(
                rows,
                SheetId,
                Settings.TableId,
                Constants.LocalizationEditorUrl,
                () => success = true
            );
