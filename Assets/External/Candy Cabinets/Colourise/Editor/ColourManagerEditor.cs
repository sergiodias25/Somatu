...
namespace CandyCabinets.Components.Colour
{
    ...
    public class ColourManagerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (colourManager.Palettes == null)
                return;
            ...
            if (serializedList.isExpanded)
            {
                if (showSize)
                    EditorGUILayout.PropertyField(
                        serializedList.FindPropertyRelative("Array.size")
                    );
                            ...
                            EditorGUILayout.PropertyField(
                                ...
                                new GUIContent(
                                    setLabel
                                        ...
                                        + (i + 1)
                                        + " | "
                                        + ColorUtility.ToHtmlStringRGB(element.colorValue)
                                        + ""
                                ),
        ...
        void UpdateSelected(ColourManager colourManager)
        {
            ...
            if (selected.Count > 1)
            {
                ...
                colourManager.SelectedPaletteIdx = colourManager.Palettes.FindIndex(
                    x => x.Selected
                );
                SetColour();
            }
        ...
        void SetColour()
        {
            List<Object> colourises = new List<Object>(
                GameObject.FindObjectsOfType(typeof(Colourise))
            );
            if (colourises.Count > 0)
            {
                colourises.ForEach(c =>
                {
                    ...
                    colourise.ColouriseComponents.ForEach(x =>
                    {
                        x.IsChanged = true;
                    });
                    colourise.ReColour();
